...
class Wrapper:
    ...
    def __init__(self, company_name: str):
        self.__company_data = data_fix.CsvHandler(company_name)
    ...
    def get_eval_model(self, start_year: int, end_year: int, pred_year: int, pred_quarters: list = None):
        # Get a list of quarters grouped by year
        years_quarters = list(range(start_year, end_year + 1)) + ['Quarter']
        # use the years of data except the last two as the training years
        training_years = years_quarters[:-2]
        # a new list of sp500 price vesus the quarters by years
        ...
        # Record the price of the first year
        ...
        # index of the year
        X = first_year_X
        # price of the corresponding date
        ...
        if pred_quarters is not None:
            pred_days = [day for day in
                         range(64 * (pred_quarters[0]-1), 64 * pred_quarters[int(len(pred_quarters) != 1)])]
        else:
